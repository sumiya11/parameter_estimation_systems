...
@parameters b1 b2 b3 b4 b5 d1 k2 k3 k4 k5 m1 m3 m4 mu2 mu3 mu4 mu5 r1 r2 r3 r4
...
@named model = ODESystem(
        [
            D(x) ~
                (r1 + b1 * y) * x * (1 - b1 * x) - d1 * x * z / (m1 + w), #pancreatic cancer cell population
            D(y) ~
                (r2 + b2 * w / (k2 + w)) * y * (1 - b2 * y) - mu2 * y, #pancreatic stellate cell population
            D(z) ~ b3 * z * v / ((k3 + v) * (m3 + w)) - mu3 * z + r3, #effector cells, including CD8+T cells and NK cells
            D(w) ~
                b4 * x * z / (k4 + x) - mu4 * w +
                r4 * x * y / (m4 + v), #concentration of tumor promoting cytokines, including TGF-beta and IL-6
            D(v) ~ b5 * x * z / (k5 + x) - mu5 * v, #concentration of tumor suppressing cytokines, including INF-gamma and IL-2
         ], t, states, parameters)
measured_quantities = [
    y1 ~ z
]
