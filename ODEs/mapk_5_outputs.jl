...
@named model = ODESystem(
	 ...
	 D(KS00) ~
                ...
                gamma1100 * FS11,
         D(KS01) ~
                -a01 * K * S01 + b01 * KS01 + c0001 * KS00 -
                alpha01 * F * S01 +
                beta01 * FS01 +
                gamma1101 * FS11,
         D(KS10) ~
                -a10 * K * S10 + b10 * KS10 + c0010 * KS00 -
                alpha10 * F * S10 +
                beta10 * FS10 +
                gamma1110 * FS11,
         D(FS01) ~
                ...
                c1011 * KS10 +
                c0011 * KS00,
		D(FS10) ~ a00 * K * S00 - (b00 + c0001 + c0010 + c0011) * KS00,
		D(FS11) ~ a01 * K * S01 - (b01 + c0111) * KS01,
          D(K) ~ a10 * K * S10 - (b10 + c1011) * KS10,
          D(F) ~ alpha01 * F * S01 - (beta01 + gamma0100) * FS01,
          D(S00) ~ alpha10 * F * S10 - (beta10 + gamma1000) * FS10,
          D(S01) ~
                alpha11 * F * S11 -
                (beta11 + gamma1101 + gamma1110 + gamma1100) * FS11,
          D(S10) ~
                -a00 * K * S00 + (b00 + c0001 + c0010 + c0011) * KS00 -
                a01 * K * S01 + (b01 + c0111) * KS01 -
               a10 * K * S10 + (b10 + c1011) * KS10,
	  D(S11) ~
                -alpha01 * F * S01 + (beta01 + gamma0100) * FS01 -
                alpha10 * F * S10 + (beta10 + gamma1000) * FS10 -
                alpha11 * F * S11 +
                (beta11 + gamma1101 + gamma1110 + gamma1100) * FS11,
	 ], t, states, parameters)
