function preprocess_ode(de::ModelingToolkit.ODESystem,
        ...
        inputs = Vector{Num}())
    @debug "Preproccessing `ModelingToolkit.ODESystem` object"
    ...
    return (SIAN.ODE{Nemo.QQMPolyRingElem}(state_eqn_dict,
            out_eqn_dict,
            inputs_),
        input_symbols, gens_)
end
...
function ode_to_poly(ode::ModelingToolkit.ODESystem, measured_quantities)
    ...
    res = identifiability_ode(ode_prep, params_to_assess_; p = 0.99,
...
function identifiability_ode(ode, params_to_assess; p = 0.99, p_mod = 0, infolevel = 0,
                             weighted_ordering = false, local_only = false)
    @debug "Solving the problem"

    p_mod = 2^30+3
    if p_mod != 0
        @debug "Using `p_mod!=0` does not guarantee the same probability of correctness but allows to run the program faster. This warning was raised by `p_mod = $p_mod`."
    end
    # 1.Construct the maximal system

    # (a) ---------------

    @debug "Constructing the maximal system"
    ...
    @debug "Assessing local identifiability"
    ...
    @debug "Found Pivots: [$(join(alg_indep, ", "))]"

    if length(theta_l) == 0
        @debug "No identifiable parameters found!"
        @debug "=== Summary ==="
        @debug "Globally identifiable parameters:                 []"
        @debug "Locally but not globally identifiable parameters: []"
        @debug "Not identifiable parameters:                      [$(join(params_to_assess, ", "))]"
        ...
        @debug "Locally identifiable parameters: [$(join([SIAN.get_order_var(th, non_jet_ring)[1] for th in theta_l], ", "))]"
        @debug "Not identifiable parameters:     [$(join([SIAN.get_order_var(th, non_jet_ring)[1] for th in setdiff(params_to_assess_, theta_l)], ", "))]"
        ...
        # 4. Determine.
        @debug "Gröbner basis computation"
        ...
        identifiability_result = Dict("globally" => Set(SIAN.get_order_var(th,
                                      ...
                                      "nonidentifiable" => Set(SIAN.get_order_var(th,
                                                               ...
                                                               for th in setdiff(params_to_assess_,
                                                                                 theta_l)))
        @debug "=== Summary ==="
        @debug "Globally identifiable parameters:                 [$(join(identifiability_result["globally"], ", "))]"
        @debug "Locally but not globally identifiable parameters: [$(join(identifiability_result["locally_not_globally"], ", "))]"
        @debug "Not identifiable parameters:                      [$(join(identifiability_result["nonidentifiable"], ", "))]"
        @debug "==============="
